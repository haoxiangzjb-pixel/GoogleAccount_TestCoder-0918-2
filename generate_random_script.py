...
def get_image_processing_code():
    """Return the code for image processing as a string."""
    code = '''import cv2
import numpy as np
import random
import string
...
def main():
    ...
    # Generate a random filename for saving the image
...
def main():
    # Get the image processing code
    code = get_image_processing_code()
    
    # Generate a random filename for the Python script
    random_script_name = generate_random_filename('.py')
    script_path = f'/workspace/{random_script_name}'
    
    # Write the code to the randomly named file
    with open(script_path, 'w') as f:
        f.write(code)
    
    print(f"Python script saved successfully as: {script_path}")
    
    # Also run the generated script
    import subprocess
    result = subprocess.run(['python', script_path], capture_output=True, text=True)
    print("Output from generated script:")
    print(result.stdout)
    if result.stderr:
        print("Errors from generated script:")
        print(result.stderr)
