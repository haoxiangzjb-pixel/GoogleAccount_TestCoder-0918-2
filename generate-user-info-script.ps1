# PowerShell脚本：生成获取当前用户信息的脚本并保存到随机命名的.ps1文件中

# 定义获取用户信息的脚本内容
$userInfoScriptContent = @'
...
# 生成随机文件名
$randomScriptFileName = "Get-UserInfo-" + (Get-Random -Maximum 99999) + ".ps1"

# 将脚本内容保存到随机命名的.ps1文件
$userInfoScriptContent | Out-File -FilePath $randomScriptFileName -Encoding UTF8

Write-Host "获取用户信息的脚本已保存到: $randomScriptFileName" -ForegroundColor Green
Write-Host "该脚本将在执行时创建另一个随机命名的文件来保存用户信息。" -ForegroundColor Yellow